...
- **Branded HTML email**, hand-built in `src/lib/email/`: table layout, inline styles, no images and no web fonts, so it renders the same in Outlook as in Apple Mail. Dark mode included, plain-text alternative on every send
- **An auto-reply** to whoever writes in, sent after the studio notification and deliberately fail-soft: if the confirmation bounces the enquiry is still reported as delivered, because it was
- **Rate limiting** on both forms, and a **Resend webhook** at `/api/webhooks/resend` that verifies its Svix signature with `node:crypto` and logs bounces, so a lost lead is visible instead of silent
...
| **Rate limiting**              | `src/lib/rateLimit.ts` caps five submissions per IP per ten minutes, but it is in-memory, so it holds within one warm serverless instance and not across them. It stops a naive flood, not a distributed one. Add a Vercel Firewall rule on `/` and `/contact`, which runs at the edge before the function is invoked. Treat as a blocker, not a follow-up |
